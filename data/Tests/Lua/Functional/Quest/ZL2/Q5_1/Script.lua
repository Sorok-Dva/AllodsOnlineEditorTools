Global( "TEST_NAME", "ZL2, Q5_1. author: Grigoriev Anton; date: 19.12.2008; Task 41636" )
...
function Step1()
	Log( "" )
	Log( "UseDevs:" )
	local devCoord = GetNoLootDevCoords( MOB_LIST, DEV1_NAME, TEST_NAME )
	UseDevs( DEV1_NAME, devCoord, DEV1_COUNT, 10000, ReturnTrue, Step2, ErrorFunc )
end
...
function Step2()
    ...
    KillMobs( QUEST_NAME, MOB2_NAME, mobCoords, StepLast, ErrorFunc )
end
...
function Init()
	...
	DEV1_COUNT = tonumber( developerAddon.GetParam( "Dev1Count" ))

	MOB2_NAME = developerAddon.GetParam( "Mob2Name" )
    MOB2_COUNT = tonumber( developerAddon.GetParam( "Mob2Count" ))
