Global( "TEST_NAME", "SmokeTest.Quest.TrackerReadyToReturn; author: Liventsev Andrey, date: 21.08.08, bug 32218" )
...
function Check()
	Log()
	Log( "all mobs killed. checking for tracker..." )
	local progress = avatar.GetQuestProgress( GetQuestId( QUEST_NAME ) )
	local obj = progress.objectives[0]
	Log( "tracker values: progress=" .. tostring(obj.progress) .. " required=" .. tostring( obj.required ) .. " count for kill=" .. tostring(COUNT_FOR_KILL))
	if obj.progress == COUNT_FOR_KILL then
