Global( "TEST_NAME", "Smoke.ShipNavigator author: Vashenko Anton; date: 11.12.2008; task: 51298" )
...
function PrintDevicesInfo( shipId )
	SHIP_ID = shipId
	PrintListDeviceShip( SHIP_ID, MoveToNavigator )
end

function MoveToNavigator()
	local deviceIndex = GetTransportDeviceIndex( SHIP_ID, SHIP_NAVIGATOR_DEV, 1 )
	DEV_ID = GetTransportDeviceId( SHIP_ID, deviceIndex )
	
	TeleportToDevice( DEV_ID, SHIP_ID, 3, UseNavigator, ErrorFunc )
end

function UseNavigator()
	StartUsingTransportDevice( DEV_ID, ZoomNavigator, ErrorFunc )
end

function ZoomNavigator()
	Log( "zoom in" )
	NavigatorZoomOnShip( SHIP_ID, UnZoomNavigator, ErrorFunc )
end

function UnZoomNavigator()
	Log( "zoom out" )
	NavigatorUnZoomOnShip( StopUsingNavigator, ErrorFunc )
end

function StopUsingNavigator()
