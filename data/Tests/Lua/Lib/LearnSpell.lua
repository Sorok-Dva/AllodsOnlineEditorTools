...
function LearnSpell( spell, funcPass, funcError)
	...
	if GetSpellId( spell ) ~= nil then
		...
		StartPrivateCheckTimer( 10000, LearnSpellCheckFunc, nil, ErrorLearnSpell, nil, PassLearnSpell, LEARN_SPELL_SPELL_ID )
	end
end
...
function ImmuneAvatar( funcPass, funcError )
	if AvatarIsImmune() == true then
		funcPass()

	else
		Log( "casting immune spell. pas== ", "Units.LearnSpell" )
